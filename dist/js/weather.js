...
$(function () {
  ...
  function setWeather(weather) {
    ...
    $ui.icon.html("<img src='http://openweathermap.org/img/wn/" + weather.icon + "@2x.png' />");
  ...
  function setForecastDay(selector, forecast) {
    ...
    $(selector + ' > div').html("<img src='http://openweathermap.org/img/wn/" + forecast.icon + "@2x.png' />");
    $(selector + ' span').eq(0).text(forecast.max.toFixed(0));
    $(selector + ' span').eq(1).text(forecast.min.toFixed(0));
  }

  function fetchWeather(lat, long) {
    let weather = null;
    const forecast = [];
    fetch('https://api.openweathermap.org/data/2.5/onecall?lat=' + lat + '&lon=' + long + '&units=metric&lang=de&APPID=' + API_KEY).then(function (response) {
      ...
      fetch('https://nominatim.openstreetmap.org/reverse?format=json&lat=' + lat + '&lon=' + long + '&zoom=10&addressdetails=1').then(function (response) {
        return response.json();
      }).then(function (data) {
